...
var minimumDeletions = function (nums) {
  function findIndexOfMinMax(nums) {
    let minIndex = 0;
    let maxIndex = 0;
    let min = nums[0];
    let max = nums[0];
  ...
  const { minIndex, maxIndex } = findIndexOfMinMax(nums);
  let biggerOne = Math.max(minIndex, maxIndex);
  let smallerOne = Math.min(minIndex, maxIndex);
  return Math.min(
    biggerOne + 1,
    nums.length - smallerOne,
    smallerOne + 1 + (nums.length - biggerOne),
  );
};

// console.log(minimumDeletions([2, 10, 7, 5, 4, 1, 8, 6])); // Output: 5
// console.log(minimumDeletions([101])); // Output: 3
...
function maxProfit(prices, fee) {
  ...
  for (let i = 1; i < prices.length; i++) {
    ...
    // Sell if we can make a profit after the fee
    if (prices[i] > buy + fee) {
      profit += prices[i] - buy - fee;

      // Important:
      // Treat today's price as the new buying price.
      buy = prices[i] - fee;
    }
  }
